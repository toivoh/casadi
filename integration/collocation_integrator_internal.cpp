...
namespace CasADi{
  ...
  CollocationIntegratorInternal::CollocationIntegratorInternal(const FX& dae, int nfwd, int nadj) : IntegratorInternal(dae,nfwd,nadj){
  ...
  void CollocationIntegratorInternal::init(){
    ...
    if(new_signature_){
      ...
      // Form derivative
      if(nfwd_>0 || nadj_>0){
        explicit_fcn_ = explicit_fcn_.derivative(nfwd_,nadj_);
      }
      ...
      if(hasSetOption("startup_integrator")){
        ...
        //        startup_integrator_ = startup_integrator_creator(dae_,0,0);
        //        startup_integrator_->new_signature_ = new_signature_;
        ...
        for(int j=0; j<coll_time_[k].size(); ++j){
          ...
          if(j>0){
            if (has_startup_integrator && startup_integrator_.hasOption("init_z")) {
              ...
              for(int i=0; i<new_signature_ ? nz_ : nfz_; ++i){
                v.at(offs++) = init_z.at(i);
              }
            } else {
              offs += new_signature_ ? nz_ : nfz_;
            }
          }
