...
namespace CasADi{
  ...
  IdasInternal::IdasInternal(const FX& dae, int nfwd, int nadj) : SundialsInternal(dae,nfwd,nadj){
  ...
  void IdasInternal::init(){
    ...
    if(hasSetOption("init_xdot") && !getOption("init_xdot").isNull()){
      ...
      casadi_assert_message(init_xdot_.size()==nfx_,"Option \"init_xdot\" has incorrect length. Expecting " << nfx_ << ", but got " << init_xdot_.size() << ". Note that this message may actually be generated by the augmented integrator. In that case, make use of the 'augmented_options' options to correct 'init_xdot' for the augmented integrator.");
    } else {
      init_xdot_.resize(nfx_);
    ...
    if(hasSetOption("init_z") && !getOption("init_z").isNull()){
      ...
      casadi_assert_message(init_z_.size()==nfz_,"Option \"init_z\" has incorrect length. Expecting " << nfz_ << ", but got " << init_z_.size() << ". Note that this message may actually be generated by the augmented integrator. In that case, make use of the 'augmented_options' options to correct 'init_z' for the augmented integrator.");
    } else {
      init_z_.resize(nfz_);
    ...
    xz_ = N_VNew_Serial(nfx_+nfz_);
    xzdot_ = N_VNew_Serial(nfx_+nfz_);
    ...
    N_Vector id = N_VNew_Serial(nfx_+nfz_);
    fill_n(NV_DATA_S(id),nfx_,1);
    fill_n(NV_DATA_S(id)+nfx_,nfz_,0);
  ...
  void IdasInternal::jtimesB(double t, const double *xz, const double *xzdot, const double *xzB, const double *xzdotB, const double *resvalB, const double *vB, double *JvB, double cjB, double * tmp1B, double * tmp2B){
    ...
    g_.getFwdSens(JvB+nfx_,RDAE_ALG);
    ...
    if(monitored("jtimesB")){
      ...
      g_.setFwdSens(JvB+nfx_,RDAE_ALG);
  ...
  void IdasInternal::resS(int Ns, double t, const double* xz, const double* xzdot, const double *resval, N_Vector *xzF, N_Vector* xzdotF, N_Vector *rrF, double *tmp1, double *tmp2, double *tmp3){
    ...
    for(int offset=0; offset<nfdir_; offset += nfdir_f_){
      ...
      for(int dir=0; dir<nfdir_batch; ++dir){
        ...
        f_.getFwdSens(NV_DATA_S(rrF[offset+dir])+nfx_,DAE_ALG,dir);
  ...
  void IdasInternal::initDenseLinearSolver(){
    // Dense jacobian
    int flag = IDADense(mem_, nfx_+nfz_);
  ...
  void IdasInternal::initBandedLinearSolver(){
    // Banded jacobian
    int flag = IDABand(mem_, nfx_+nfz_, getOption("upper_bandwidth").toInt(), getOption("lower_bandwidth").toInt());
  ...
  FunctionType IdasInternal::getJacobianGen(){
    ...
    typename FunctionType::MatType jac = f.jac(DAE_X,DAE_ODE) - cj*FunctionType::MatType::eye(nfx_);
    if(nfz_>0){
